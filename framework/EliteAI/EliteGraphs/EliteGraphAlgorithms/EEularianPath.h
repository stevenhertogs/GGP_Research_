...
namespace Elite
{
	...
	inline vector<T_NodeType*> EulerianPath<T_NodeType, T_ConnectionType>::FindPath(Eulerianity& eulerianity) const
	{
		...
		{
			for (int i{}; graphCopy->GetNrOfNodes(); i++)
			{
		...
		while (graphCopy->GetNodeConnections(index).size() != 0)
		{
			T_ConnectionType* pConnection = graphCopy->GetNodeConnections(index).front();
			index = pConnection->GetTo();
			path.push_back(m_pGraph->GetNode(index));
